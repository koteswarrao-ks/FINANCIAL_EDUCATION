...
def profile_agent_fn(child_id: str) -> dict:
    """
    Profile agent using LangChain tools for MCP operations and RAG retrieval.
    Uses structured tools instead of direct HTTP calls.
    """

    # 1️⃣ Fetch data from MCP using tool
    data = get_user_profile.invoke({"child_id": child_id})
    ...
    # 2️⃣ Semantic search from vector DB using RAG tool
    # Build semantic query from transaction data
    ...
    try:
        ...
        retrieved_docs = retrieve_financial_concepts.invoke({
...
""")
    ...
    # 5️⃣ Merge with user preferences using tool
    try:
        preferences = get_profile_preferences.invoke({"child_id": child_id})
